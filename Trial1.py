...
def frame_disc():
    ...
    def d_layer(layer_input, filters, f_size=4, bn=True):
        ...
        d = TimeDistributed(Conv2D(filters//2, kernel_size=f_size, strides=1, padding='same', kernel_regularizer=l2(0.01)))(layer_input)
        d = TimeDistributed(LeakyReLU(alpha=0.2))(d)
        d = TimeDistributed(Conv2D(filters, kernel_size=f_size, strides=2, padding='same', kernel_regularizer=l2(0.01)))(d)
        d = TimeDistributed(LeakyReLU(alpha=0.2))(d)
        if bn:
            d = TimeDistributed(BatchNormalization(momentum=0.8))(d)
        res = TimeDistributed(Conv2D(filters, kernel_size=f_size, strides=2, padding='same', kernel_regularizer=l2(0.01)))(layer_input)
        d = add([d, res])
        return d
    ...
    d8 = TimeDistributed((Dense(1,activation='sigmoid', kernel_regularizer=l2(0.01))))(d7)
    disc = Model(img_A,d8)
    return disc
...
def vid_disc():
    ...
    def d_layer(layer_input, filters, f_size=4, bn=True):
        """Layers used during downsampling"""
        d = Conv3D(filters//2, kernel_size=f_size, strides=1, padding='same', kernel_regularizer=l2(0.01))(layer_input)
        d = LeakyReLU(alpha=0.2)(d)
        d = Conv3D(filters, kernel_size=f_size, strides=2, padding='same', kernel_regularizer=l2(0.01))(d)
        d = LeakyReLU(alpha=0.2)(d)
        if bn:
            d = BatchNormalization(momentum=0.8)(d)
        res = Conv3D(filters, kernel_size=f_size, strides=2, padding='same', kernel_regularizer=l2(0.01))(layer_input)
        d = add([d, res])
        return d
    ...
    d5 = d_layer(d4, df*8)
    flat = Flatten()(d5)
    validity = Dense(1, activation='sigmoid', kernel_regularizer=l2(0.01))(flat)
...
def extractor():
    ...
    def conv2d(layer_input, filters, f_size=4, bn=True):
        """Layers used during downsampling"""
        d = Conv2D(filters//2, kernel_size=f_size, strides=1, padding='same', kernel_regularizer=l2(0.01))(layer_input)
        d = LeakyReLU(alpha=0.2)(d)
        d = Conv2D(filters, kernel_size=f_size, strides=1, padding='same', kernel_regularizer=l2(0.01))(d)
        d = LeakyReLU(alpha=0.2)(d)
        if bn:
            d = BatchNormalization(momentum=0.8)(d)
        res = Conv2D(filters, kernel_size=f_size, strides=1, padding='same', kernel_regularizer=l2(0.01))(layer_input)
        d = add([d, res])
        return d
    def conv3d(layer_input, filters, f_size=4, bn=True):
        ...
        d = Conv3D(filters//2, kernel_size=f_size, strides=1, padding='same', kernel_regularizer=l2(0.01))(layer_input)
        d = LeakyReLU(alpha=0.2)(d)
        d = Conv3D(filters, kernel_size=f_size, strides=(1,2,2), padding='same', kernel_regularizer=l2(0.01))(d)
        d = LeakyReLU(alpha=0.2)(d)
        if bn:
            d = BatchNormalization(momentum=0.8)(d)
        res = Conv3D(filters, kernel_size=f_size, strides=(1,2,2), padding='same', kernel_regularizer=l2(0.01))(layer_input)
        d = add([d, res])
        return d
    ...
    def deconv3d(layer_input, skip_input, filters, f_size=4, dropout_rate=0):
        ...
        u = Conv3D(filters//2, kernel_size=f_size, strides=1, padding='same', activation='relu', kernel_regularizer=l2(0.01))(u1)
        u = Conv3D(filters, kernel_size=f_size, strides=1, padding='same', activation='relu', kernel_regularizer=l2(0.01))(u)
        if dropout_rate:
            u = Dropout(dropout_rate)(u)
        u = BatchNormalization(momentum=0.8)(u)
        u = Concatenate()([u, skip_input])
        res = Conv3D(filters*2, kernel_size=f_size, strides=1, padding='same', activation='relu', kernel_regularizer=l2(0.01))(u)
        u = add([u, res])
        return u
    ...
    output_vid = Conv3D(channels, kernel_size=4, strides=1, padding='same', activation='sigmoid',name='conv6', kernel_regularizer=l2(0.01))(u7)
...
def build_generator():
    ...
    def conv2d(layer_input, filters, f_size=4, bn=True):
        ...
        d = Conv2D(filters//2, kernel_size=f_size, strides=1, padding='same', kernel_regularizer=l2(0.01))(layer_input)
        d = LeakyReLU(alpha=0.2)(d)
        d = Conv2D(filters, kernel_size=f_size, strides=2, padding='same', kernel_regularizer=l2(0.01))(d)
        d = LeakyReLU(alpha=0.2)(d)
        if bn:
            d = BatchNormalization(momentum=0.8)(d)
        res = Conv2D(filters, kernel_size=f_size, strides=2, padding='same', kernel_regularizer=l2(0.01))(layer_input)
        d = add([d, res])
        return d
    ...
    def deconv2d(layer_input, skip_input, filters, f_size=4, dropout_rate=0):
        ...
        u = Conv2D(filters//2, kernel_size=f_size, strides=1, padding='same', activation='relu', kernel_regularizer=l2(0.01))(u1)
        u = Conv2D(filters, kernel_size=f_size, strides=1, padding='same', activation='relu', kernel_regularizer=l2(0.01))(u)
        if dropout_rate:
            u = Dropout(dropout_rate)(u)
        u = BatchNormalization(momentum=0.8)(u)
        u = Concatenate()([u, skip_input])
        res = Conv2D(filters*2, kernel_size=f_size, strides=1, padding='same', activation='relu', kernel_regularizer=l2(0.01))(u)
        u = add([u, res])
        return u
...
#    u2 = deconv2d(d6, d5, gf*16)
    ...
    output_img = Conv2D(channels, kernel_size=4, strides=1, padding='same', activation='sigmoid',name='conv6', kernel_regularizer=l2(0.01))(u7)
...
def build_discriminator():
    ...
    def d_layer(layer_input, filters, f_size=4, bn=True):
        """Discriminator layer"""
        d = Conv2D(filters, kernel_size=f_size, strides=2, padding='same', kernel_regularizer=l2(0.01))(layer_input)
        d = LeakyReLU(alpha=0.2)(d)
        if bn:
            d = BatchNormalization(momentum=0.8)(d)
        return d
...
images_path = "D:\Capstone\Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks\DIV2K_train_HR"
...
for filename in images_name:
    ...
    imag = imag.resize([64,64], Image.Resampling.LANCZOS)
    imag = (np.array(imag) - 127.5 / 127.5)
    datasetlist.append(imag)
    # f = open('3dgan/imgs.txt','a+')
    f = open('D:/Capstone/Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks/threedgan/imgs.txt', 'a+')
    f.write( ' \n'+ filename)
    f.close()
...
# loc = "./DATA/"
loc = "D:/Capstone/Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks/DATA/"
vid_paths = os.listdir(loc)
x1 = np.zeros((50,64,64,3))
x1 = np.stack((x1,x1))
for path in vid_paths:
    #vid_paths contains each video of 50 frames, we need to create these sub directories of each video
    frames = os.listdir(loc+path+'/img/')
    vid = []
    cnt = 0
    print('Inputting video')
    for i in range(50):
        # here the code is opening the frames of the video, but the video is not brokendown, so we need to do that 
        if i < 9:
...
#        f_loss = featured.train_on_batch(x,feature_x)
    ...
    if (epoch%100 ==0) :
        ...
        if (epoch%1000 ==0) :
            ...
            gen.save_weights("D:\Capstone\Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks\\threedgan\Models\gen"+str(epoch)+".weights.h5")
            print("Saved generator model to disk")
            # ex.save_weights("./3dgan/Models/ex"+str(epoch)+".h5")
            ex.save_weights("D:\Capstone\Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks\\threedgan\Models\ex"+str(epoch)+".weights.h5")
...
#            plt.imshow(cv2.cvtColor(c_img[0], cv2.COLOR_RGB2BGR))
        ...
        fig.savefig(r'D:\Capstone\Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks\threedgan\Models\Test_epoch'+str(epoch)+'.png')
...
fig.savefig('D:\\Capstone\\Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks\\threedgan.loss_curves.png')


arg = np.argmin(select1)
g_model_arg = arg*1000
# gen.load_weights('./3dgan/Models/gen'+str(g_model_arg)+'.h5')
gen.load_weights('D:\Capstone\Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks\\threedgan\Models\gen'+str(g_model_arg)+'.weights.h5')
arg = np.argmin(select2)
model_arg = arg*1000
# ex.load_weights('./3dgan/Models/ex'+str(model_arg)+'.h5')
ex.load_weights('D:\Capstone\Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks\\threedgan\Models\ex'+str(model_arg)+'.weights.h5')
...
for i in range(len(enc_imgs)):
    ssim_ci = ssim(X[i].reshape(64, 64, 3), enc_imgs[i].reshape(64, 64, 3).astype('float64'), win_size=3, data_range=enc_imgs[i].max() - enc_imgs[i].min(), multichannel=True)
    woow.append(ssim_ci)
    
# file-output.py
# f = open('3dgan/results_video.txt','a+')
f = open('D:/Capstone/Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks/threedgan/results_video.txt','a+')
...
printVid('D:\Capstone\Hiding-Video-in-Images-using-Deep-Generative-Adversarial-Networks\threedgan\Models\Results',1,vid_pred,vid_act,motion_vid)
